...
# Multi-seed runners
def run_multi_seed(seed_base=2025, num_runs=10, save_dir='LSTM'):
    ...
    for i in range(num_runs):
        ...
        model = LSTMpred(input_size=1, hidden_size=256, num_layers=2).to(device)

        cfg = dict(config)
        cfg['save_path'] = os.path.join(save_dir, f"model_lstm_seed{seed}.pth")
    ...
    plt.ylabel("Test MSE")
    plt.title(f"LSTM mean ± 95% CI (n={num_runs})")
    plt.tight_layout()
    plt.show()

    return {"scores": scores, "mean_mse": mean_mse, "std_mse": sd_mse, "ci95": ci95, "ckpts": ckpts, "csv": csv_path}

def run_multi_seed_eval_only(seed_base=2025, num_runs=10, save_dir='LSTM',
                   ckpt_tmpl="model_lstm_seed{seed}.pth"):
    os.makedirs(save_dir, exist_ok=True)

    dataset_tt = StrainDataset(test_csv, 'test', seq_len, transforms=None)
    model_tt_data = DataLoader(dataset_tt, batch_size=batch_size,
    ...
    plt.bar(["LSTM"], [mean_mse], yerr=[ci95], capsize=6)
    plt.ylabel("Test MSE")
    plt.title(f"LSTM mean ± 95% CI (n={len(scores)})")
    plt.tight_layout()
    plt.show()
