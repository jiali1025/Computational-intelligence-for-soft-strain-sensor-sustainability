...
class _MixerBlock(nn.Module):
    def __init__(self, seq_len, hidden_dim, token_mlp_dim, channel_mlp_dim, dropout=0.):
        super().__init__()
        self.ln1 = nn.LayerNorm(hidden_dim)
        self.token_mlp = _FFN(seq_len, token_mlp_dim, dropout)
        self.ln2 = nn.LayerNorm(hidden_dim)
        self.chan_mlp  = _FFN(hidden_dim, channel_mlp_dim, dropout)
    def forward(self, x):     # x: [B, T, C]
        y = self.ln1(x).transpose(1, 2)   # [B, C, T]
        y = self.token_mlp(y)             # token-mixing
        x = x + y.transpose(1, 2)
        y = self.chan_mlp(self.ln2(x))    # channel-mixing
        return x + y

class MLPMixerTSRegressor(nn.Module):

    def __init__(self, input_dim=2, seq_len=30, hidden_dim=128, depth=4,
                 token_mlp_dim=64, channel_mlp_dim=256, dropout=0.1):
        super().__init__()
        self.proj = nn.Linear(input_dim, hidden_dim)
        self.blocks = nn.ModuleList([
            _MixerBlock(seq_len, hidden_dim, token_mlp_dim, channel_mlp_dim, dropout)
...
model = MLPMixerTSRegressor(input_dim=2, seq_len=30, hidden_dim=64, depth=2)
...
os.makedirs('Durability_MLPMixer_01', exist_ok=True)
config = {
        'n_epochs': 100,
        'batch_size': 36,
        'optimizer': 'SGD',
        ...
        'early_stop': 100,
        'save_path': 'Durability_MLPMixer_01/Durability_MLPMixer_01.pth'
}
...
print(preds01)
print(test_loss01)
# save_pred(preds01, 'Durability_MLPMixer_01_Preds.csv')  # save prediction file to pred.csv
