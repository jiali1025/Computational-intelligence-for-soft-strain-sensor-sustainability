...
os.makedirs('Durability_D_Former_ablation03_1', exist_ok=True)
config = {
    'n_epochs': 10,
    'early_stop': 100,
    'save_path': 'Durability_D_Former_ablation03_1/Durability_D_Former_01.pth'
}
...
def train(model_tr_data, dv_set, model, config, device):
    ...
    if len(lrs) > 0:
        plt.plot(lrs)
        plt.title('Learning rate schedule (first param group)')
        plt.show()
...
save_pred(preds01, targets01, 'Durability_D_Former_ablation03_Preds_01.csv')
