# D_Former without causal_convolution_layer
import csv
...
model = D_Former_NoConv(d_model, nhead, d_hid, nlayers, dropout).to(device)

os.makedirs('Durability_D_Former_ablation02', exist_ok=True)
config = {
    'n_epochs': 10,
    'early_stop': 100,
    'save_path': 'Durability_D_Former_ablation02/Durability_D_Former_01.pth'
}
...
def plot_learning_curve(loss_record, title=''):
    ...
    plt.ylabel('MSE loss')
    plt.title('Learning curve (no causal conv)')
    plt.legend()
    plt.show()

# plot_learning_curve(model_loss_record, title='D_Former_NoConv')
...
save_pred(preds01, targets01, 'Durability_D_Former_ablation02_Preds_1.csv')
