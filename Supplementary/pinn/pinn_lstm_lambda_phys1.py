...
class StrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        if index >= self.seq_len - 1:
            ...
            x = self.X[i0:index+1, :]  # resistance sequence
            t = self.T[i0:index+1, :]
        else:
...
class PINNLSTM(nn.Module):
    ...
    def forward(self, x):  # x: (B,L,1) resistance sequence
        out, _ = self.lstm(x)
...
def main():
    cfg = {
        ...
        'hidden_size': 256,
        'num_layers': 2,
        'lambda_phys': 1.0,  # physics loss weight; tune ~0.1 to 5
        'dev_ratio': 0.1,
    }
    train_csv = './cycle_180_190_time.csv'
    test_csv  = './cycle_190_195_test.csv'
    seeds = [0,1,2,3,4]
    base_out = 'runs_pinn_lstm_simple_lp1'
