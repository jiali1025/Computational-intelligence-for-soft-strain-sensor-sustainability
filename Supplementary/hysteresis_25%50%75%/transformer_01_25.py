...
class StrainDataset(Dataset):
    """
        Dataset for sequence-to-one regression:
        Input: a sliding window of resistance values (seq_len x 1)
    """
    def __init__(self, path, mode='train', sequence_length=500, transforms=None):
        ...
        with open(path, 'r') as fp:
            ...
            data = np.array(data)
            features = data[1::3, 2:3]   # resistance (1D feature)
            target = data[1::3, 4:5]     # regression target
...
def train_dev_data(x_set):
    """Simple split: 90% train (i%10!=0) and 10% dev (i%10==0)."""
    X = []
    deX = []
...
# Train/test datasets
dataset_for_train = StrainDataset('./25cycle_le10_40_45_.csv', 'train', 200, transforms=None)
dataset_for_test = StrainDataset('./25cycle_40_45.csv', 'test', 200, transforms=None)
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, nhead: int, d_hid: int, nlayers: int, dropout: float = 0):
        ...
        # batch_first=True => input/output: (B, T, C)
    ...
    def forward(self, src: Tensor) -> Tensor:
        src = self.prenet(src)                 # (B, T, 1) -> (B, T, d_model)
...
os.makedirs('Transformer_25', exist_ok=True)
config = {
    'n_epochs': 120,
    'early_stop': 100,
    'save_path': 'Transformer_25/model_transformer_01.pth'
}
...
def train(model_tr_data, dv_set, model, config, device):
    """Training loop with early stopping based on dev MSE."""
...
def dev(dv_set, model, device):
    """Evaluate average MSE on the dev set."""
    model.eval()
    total_loss = 0
...
def test(tt_set, model, device):
    """Run inference on the test set and report average MSE."""
    model.eval()
    preds = []
    targets = []
    test_loss = 0.0
...
save_pred(preds, targets, '25pred_transformer_01.csv')
print('ALL Done!')
