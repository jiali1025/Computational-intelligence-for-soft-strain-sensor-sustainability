...
dataset_for_train = StrainDataset('./pressure_train_cycles_0-24.csv', 'train', 500, transforms=None)
dataset_for_test_01 = StrainDataset('./pressure_test_cycles_25-2478.csv', 'test', 500, transforms=None)
...
os.makedirs('Pressure25_D_Former_01', exist_ok=True)
config = {
    'n_epochs': 120,
    'early_stop': 100,
    'save_path': 'Pressure25_D_Former_01/Durability_D_Former_01.pth'
}
...
def train(model_tr_data, dv_set, model, config, device):
    ...
    while epoch < n_epochs:
        ...
        model.eval()
        x, y, *_ = next(iter(model_tr_data))
        with torch.no_grad():
            p = model(x.float().to(device)).detach().cpu().numpy().reshape(-1)
...
def test(tt_set, model, device):
    ...
    for x, y, t, r, c in tt_set:
        ...
        with torch.no_grad():
            pred = model(x)
            # pred = pred * dataset_for_train.y_std + dataset_for_train.y_mean
            # y = y * dataset_for_test_01.y_std + dataset_for_test_01.y_mean
    ...
    # targets = targets * dataset_for_test_01.y_std + dataset_for_test_01.y_mean
    # loss1= criterion(preds, targets)
...
# save_pred(preds01_raw, targets01_raw, 'Pressure25_D_Former_01_Preds1.csv')
