...
dataset_for_train = StrainDataset('./Training_batchdiversity_1589filtered1000.csv', 'train', 500, transforms=None)
...
class D_Former(nn.Module):
    def __init__(self, d_model: int):
        ...
        config = RoFormerConfig(hidden_size=d_model,
...
# os.makedirs('BatchDiversity_1000cycles_D_Former_1589models', exist_ok=True)
config = {
    'n_epochs': 100,
    'early_stop': 100,
    'save_path': 'BatchDiversity_1000cycles_D_Former_1589models/BatchDiversity_D_Former_01.pth'
}
...
class MAPELoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        loss = torch.mean(torch.abs((targets - predictions) / 50 ))  # max_strain = 50.0
        return loss
...
# save_pred(preds01, targets01, 'BatchDiversity_1000cycles_D_Former_01.csv')
