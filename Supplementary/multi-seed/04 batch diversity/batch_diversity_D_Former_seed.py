...
class D_Former(nn.Module):
    ...
    def forward(self, src: Tensor) -> Tensor:
        B = src.size(0)
        src = self.prenet(src)                        # [B, 500, d_model]
        z_embedding = self.input_embedding(src)
        z_embeoutput = z_embedding.reshape(B, -1, 24) # [B, L, 24]
...
os.makedirs('BatchDiversity_D_Former_models', exist_ok=True)
config = {
    'n_epochs': 100,
    'early_stop': 100,
    'save_path': 'BatchDiversity_D_Former_models/BatchDiversity_D_Former_01.pth'
}
...
def run_one_seed(seed: int):
    ...
    print(f"[SEED {seed}]  | test_mse={test_mse:.6f} | test_mape={test_mape:.6f}")
    return seed, test_mse, test_mape, seed_ckpt_path
...
def main():
    ...
    summary_rows = []
    for s in seeds:
        seed, test_mse, test_mape, ckpt_path = run_one_seed(s)
        summary_rows.append([seed, test_mse, test_mape, ckpt_path])
    ...
    with open(summary_csv, 'w', newline='') as fp:
        ...
        writer.writerow(['seed', 'test_mse', 'test_mape', 'ckpt_path'])
