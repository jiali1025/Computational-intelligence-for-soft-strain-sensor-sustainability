...
# dev/test/train functions (adapted to use the preserved model code)
...
def train(tr_loader, dv_set, model, config, device, save_path=None):
    ...
    # warmup / total steps
    # NB: we will compute warmup_steps and total_steps using dataset lengths passed in run
    # If scheduler steps per batch used, we will handle that in run_one_seed to create scheduler.
    min_mse = 2000
    ...
    # This function will be wrapped by run_one_seed which provides scheduler stepping.
    while epoch < n_epochs:
        # training loop body will be executed within run_one_seed where optimizer and scheduler are controlled
        # But keep a fallback simple loop here in case it's called standalone.
        allbat_mse_loss = 0
        model.train()
...
def run_one_seed(seed, tr_loader, dv_loader, tt_loader, config, save_dir):
    ...
    ckpt_path = os.path.join(save_dir, f"Durability_Transformer_seed{seed}.pth")

    # optimizer + scheduler
    optimizer = AdamW(model.parameters(), lr=config.get('lr', 1e-5))
    total_steps = len(tr_loader) * config['n_epochs']
    warmup_steps = max(1, len(tr_loader) * 2)  # 2 epochs warmup as earlier
    ...
    # load ckpt to test best
    # model.load_state_dict(torch.load(ckpt_path))
    preds, test_mse = test(tt_loader, model, device)
    return test_mse, ckpt_path, loss_record

# multi-seed runner
def run_multi_seed_transformer(seed_base=2025, num_runs=10,
                               train_csv='./01_training_data_4000.csv',
                               test_csv='./01_testing_data_16000.csv',
                               seq_len=500, batch_size=32, save_dir='Durability_Transformer_01'):
    # prepare datasets + loaders (keep your original split logic)
    ...
    plt.ylabel("Test MSE")
    plt.title(f"Transformer mean ± 95% CI (n={num_runs})")
    plt.tight_layout()
    plt.show()
...
def run_multi_seed_eval_only(
        ...
        seq_len=500,
        batch_size=32,
        save_dir='Durability_Transformer_01',
        ckpt_tmpl='Durability_Transformer_seed{seed}.pth'
    ):

    os.makedirs(save_dir, exist_ok=True)
    dataset_for_test = StrainDataset(test_csv, 'test', seq_len, transforms=None)
    tt_loader = DataLoader(dataset_for_test, batch_size=batch_size,
    ...
    for i in range(num_runs):
        seed = seed_base + i
        print("\n" + "=" * 60)
        print(f"[Eval Run {i+1}/{num_runs}] seed={seed}")
        set_seed(seed)
