...
def run_one_seed(seed, config, tr_loader, dv_loader, tt_loader, ckpt_dir):
    ...
    ckpt_path = os.path.join(ckpt_dir, f"Durability_1DCNN_seed{seed}.pth")
...
def run_multi_seed(seed_base=2025, num_runs=10,
                   ...
                   seq_len=30,
                   batch_size=36,
                   save_dir="Durability_1DCNN_multi_seed"):
    # DataLoader
    ...
    plt.ylabel("Test MSE")
    plt.title(f"1D-CNN (n={num_runs}) mean ± 95% CI")
    plt.tight_layout()
    plt.show()
...
def run_multi_seed_eval_only(
        ...
        seq_len=30,
        batch_size=36,
        ckpt_dir="Durability_1DCNN_01",
        ckpt_tmpl="Durability_1DCNN_seed{seed}.pth"
    ):
