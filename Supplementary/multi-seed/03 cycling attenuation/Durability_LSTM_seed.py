...
class LSTMpred(nn.Module):
    ...
    def init_hidden(self):
        ...
        return (torch.zeros(self.num_layers, 64, self.hidden_size).to(device),
			    torch.zeros(self.num_layers, 64, self.hidden_size).to(device))
...
model = LSTMpred(input_size=2, hidden_size=64, num_layers=2, batch_first=True)
...
def run_one_seed(seed, config, tr_loader, dv_loader, tt_loader, ckpt_dir):
    set_seed(seed)
    model = LSTMpred(input_size=2, hidden_size=64, num_layers=2, batch_first=True).to(device)
    ckpt_path = os.path.join(ckpt_dir, f"Durability_LSTMpred_seed{seed}.pth")
...
def run_multi_seed(seed_base=2025, num_runs=10,
                   ...
                   seq_len=30,
                   batch_size=64,
                   save_dir="Durability_LSTMpred_multi_seed"):
    # DataLoader
    ...
    plt.bar(["LSTM"], [mean_mse], yerr=[ci95], capsize=6)
    plt.ylabel("Test MSE")
    plt.title(f"LSTM (n={num_runs}) mean ± 95% CI")
    plt.tight_layout()
    plt.show()
...
def run_multi_seed_eval_only(
        ...
        seq_len=30,
        batch_size=64,
        ckpt_dir="Durability_LSTM_01",
        ckpt_tmpl="Durability_LSTMpred_seed{seed}.pth"
    ):
    ...
    mean_mse, sd_mse, ci95 = mean_std_ci95(scores)
    print("\n===== Multi-seed Summary (LSTM, EVAL ONLY) =====")
    ...
    plt.bar(["LSTM"], [mean_mse], yerr=[ci95], capsize=6)
    plt.ylabel("Test MSE")
    plt.title(f"LSTM (n={len(scores)}) mean ± 95% CI (EVAL ONLY)")
    plt.tight_layout()
    plt.show()
