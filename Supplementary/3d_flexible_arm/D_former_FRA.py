...
dataset_for_train = StrainDataset('./3Ddata_40.csv', 'train', 500, transforms=None)
dataset_for_test_01 = StrainDataset('./3Ddata_60.csv', 'test', 500, transforms=None)
...
class D_Former(nn.Module):
    def __init__(self, d_model: int, dropout: float = 0):
        ...
        self.prenet = nn.Linear(6, d_model)
...
model = D_Former(d_model, dropout).to(device)

os.makedirs('Appli_Dformer_3D', exist_ok=True)
config = {
    'n_epochs': 150,
    'early_stop': 100,
    'save_path': 'Appli_Dformer_3D/robotic_arm_D_Former_01.pth'
}

criterion = nn.MSELoss()


def mae_3d(output, target):


    true_coords = target.cpu().numpy()
    predicted_coords = output.cpu().numpy()
    # print("true shape:", true_coords.shape, "pred shape:", predicted_coords.shape)
...
preds01, targets01, test_loss01, test_mape01 = test(model_tt_dataX01, model, device)  # predict strain cases with your model
...
save_pred(preds01, targets01, 'robotic_arm_D_Former3D_Preds01.csv')
